...
ポイント：`value(1)` で点灯、`value(0)` で消灯し、`time.sleep` で待つだけです。三項演算子や
`global` などは使っておらず、[入門編](30_M5Stack実装に向けたPython入門.md)の知識
（import・メソッド呼び出し・try/except）と入門I の基礎だけで読めます。
終了時（Ctrl-C）には `led.value(0)` で消灯してから終わります。
