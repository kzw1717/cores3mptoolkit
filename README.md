...
```
PC で .py を書く  →  USB で CoreS3 に接続  →  python -m mpremote run で実行  →  Ctrl-C で停止
```
...
これらは **すべてのサンプル（Grove 11本・CoreS3 21本の全32本）** に共通で出てくるため、
...
| 条件付き表式（三項演算子） | [IM2] [battery_status.py](https://github.com/kzw1717/cores3mptoolkit/blob/main/m5stack_samples/battery_status.py) |
| 数値計算（`math`・小数表示） | [IG7] [temperature_sensor.py](https://github.com/kzw1717/cores3mptoolkit/blob/main/Level1_samples/temperature_sensor.py) |
...
| 解説ページ | 内容 |
| --- | --- |
| [docs/10_Grove_入力サンプル.md](docs/10_Grove_入力サンプル.md) | 傾き・タッチ・土壌・光・水・音・温度・距離などの**読み取り**（8本） |
| [docs/11_Grove_出力サンプル.md](docs/11_Grove_出力サンプル.md) | LED・振動モーターなど**光らせる/動かす**出力（3本） |
